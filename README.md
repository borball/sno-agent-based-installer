...
- sno-iso: Used to generate bootable OCP image based on [OpenShift Agent Based Installer](https://docs.openshift.com/container-platform/4.12/installing/installing_with_agent_based_installer/preparing-to-install-with-agent-based-installer.html)
- sno-install: Used to mount the ISO generated by sno-iso.sh to BMC console with Redfish API, and boot the node from the image. 
- sno-day2: Used to apply day2 configurations required by vDU.
- sno-ready: Used to validate if the cluster has all required tunings.

Note: 

This repo only works for OpenShift 4.12+. For 4.9/4.10 please use this [helper script](https://github.com/borball/sno-manual-helper)
